...
function optometry(identityCard) {
	var page = layer.open({
		type : 2,
		title : '电脑验光数据',
		maxmin : true,
		shadeClose : false, // 点击遮罩关闭层
		area : [ '450px', '520px' ],
		content : prefix + '/optometry/' + identityCard // iframe的url
	});
	layer.full(page);
}
...
function daoru(){
	...
	$.ajax({
		cache : true,
		type : "POST",
		url : "/xinshineng/shaicha/importMember",
...
function daorudatijiguo(){
	...
	$.ajax({
		cache : true,
		type : "POST",
		url : "/xinshineng/shaicha/daorudatijiguo",
		data : formData,// 你的formid
		processData:false,
		contentType:false,
		async : false,
...
function erweimaxiazai(){
	...
	window.location.href="/xinshineng/shaicha/downloadErweima?ids="+ids
}
...
function shaichajieguodaochu(){
	...
	window.location.href="/xinshineng/shaicha/shifanshaichajieguodaochu?ids="+ids
	
}

function erweimaxiazaibyliulanqi(id){
	//window.location.href="/information/student/downLoadErWeiMaByBrower?id="+id
	window.open("/xinshineng/shaicha/downLoadErWeiMaByBrower?id="+id);
}

function shifanshaichadayin(id,lastCheckTime){
	if(lastCheckTime=='null'){
		alert('没有进行过筛查检查');
		return ;
	}
	window.open("/xinshineng/shaicha/shifanshaichadayin?id="+id);
}

function checkdetail(identityCard){
	window.location.href="/xinshineng/shaicha/shifanshaichadetail?identityCard="+identityCard;
}
